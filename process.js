...
const RRESLT = /^\{(.+?) threads\} -> \{(.+?)ms, (.+?)ms mark, (.+?) init, (.+?)ms first, (.+?)ms move, (.+?)ms aggr, (.+?) slots, (.+?) iters, (.+?) passes, (.+?) modularity\} (.+)/m;
...
function readLogLine(ln, data, state) {
  ...
  if (ROMPTH.test(ln)) {
    ...
    var [, num_threads, time, marking_time, initialization_time, first_pass_time, local_moving_phase_time, aggregation_phase_time, num_slots, iterations, passes, modularity, technique] = RRESLT.exec(ln);
